            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-6 mt-4 ">
                {
                    loadedBooks.map(book => <div key={book._id} className={`flex max-w-md flex-col justify-center items-center md:flex-row lg:flex-row overflow-hidden bg-white rounded-lg shadow-lg p-3 hover:-translate-y-2 duration-700 ${theme === "light" ? "bg-white" : "bg-[#313332] text-white"}`}>
                        ...
                        <div className="md:w-2/3 lg:w-2/3 h-fit p-4 md:p-4 text-center md:text-left lg:text-left">
                            <h1 className=" text-sm md:text-sm lg:text-lg font-bold font-lob">{book.book_name}</h1>
                            <h1 className="text-xs md:text-xs lg:text-sm mt-2 font-normal  "> <span className="font-semibold">Author : </span>{book.book_author}</h1>
                            ...
                            <div className="flex mt-3 items-center flex-col md:flex-col lg:flex-col xl:flex-row justify-between">
                                <h1 className="text-sm mt-2 font-normal    flex justify-center items-center gap-2"> <span className="font-light flex justify-center items-center gap-2">Rating : <span className="font-normal">{book.book_rating}</span></span><FaStar className="text-orange-400"></FaStar></h1>
                                <h1 className="text-xs md:text-xs lg:text-sm mt-2 font-normal  "> <span className="font-medium">Quantity : </span >{book.book_quantity}</h1>
                            </div>
